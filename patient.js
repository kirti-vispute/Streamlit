...
import { addDoc, collection, doc, getDoc, getDocs, getFirestore, onSnapshot, orderBy, query, serverTimestamp, updateDoc, where } from "https://www.gstatic.com/firebasejs/10.4.0/firebase-firestore.js";
...
const firebaseConfig = {
    ...
    measurementId: "G-Y0CDSS5NWD"
        ...
        document.getElementById('booking-form').addEventListener('submit', async (e) => {
            ...
            // Map practitioner to a doctor user in Firestore by name if available
            const practitionerName = document.getElementById('appointment-practitioner').value;
            let doctorId = '';
            try {
                // naive mapping: search users where role=='staff' and name matches
                // If your users collection stores name, you can fetch all staff and find by name
                // Keeping minimal reads: attempt to map a few known names → optional
                const nameToId = {};
                // Optionally prefill via last login email stored by staff when using doctor portal — skipped here
                // Fallback: leave doctorId empty if not resolved
                // You can extend this mapping via an admin UI later.
                if (nameToId[practitionerName]) doctorId = nameToId[practitionerName];
            } catch(_) {}

            // Convert time to 24h string like '14:30' for doctor portal mapping
            function to24h(t){
                const m = String(t).trim();
                const ampm = /am|pm/i.test(m) ? m.match(/am|pm/i)[0].toLowerCase() : '';
                const [hh, mm] = m.replace(/am|pm/ig,'').trim().split(':');
            ...
            const appointmentData = {
                ...
                time: to24h(document.getElementById('appointment-time').value),
        ...
        async function fetchAndDisplayAppointments() {
            ...
            const qRef = query(collection(db, "appointments"), where("userId", "==", user.uid), orderBy("date", "desc"));
            try {
                apptUnsub = onSnapshot(qRef, (querySnapshot) => {
                    if (querySnapshot.empty) {
                        ...
                        const btn = document.getElementById('appt-book-now-btn');
                        if (btn) btn.addEventListener('click', () => { renderTreatments(); showPage('treatments-section', 'Our Treatments'); });
                        return;
                    }
                    ...
                    querySnapshot.forEach((d) => {
                        ...
                        if (apptDate >= today && appt.status !== 'Cancelled') { upcomingAppointments.push(appt); }
                        else { pastAppointments.push(appt); }
                    });
