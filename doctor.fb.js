...
import { getFirestore, collection, doc, getDoc, onSnapshot, orderBy, query, updateDoc, where, getDocs, setDoc } from "https://www.gstatic.com/firebasejs/10.4.0/firebase-firestore.js";
...
function subscribeAppointments(callback, statusFilter = null){
  const baseQ = query(collection(db, 'appointments'), orderBy('date', 'desc'));
  return onSnapshot(baseQ, async snap => {
    const items = [];
